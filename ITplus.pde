    ...
    if (rf12_recvDone()) {
        ...
        if (1 == 0) {
          ...
          Serial.print(" ?");
            ...
            activityLed(1);
			Serial.print('@');
			printByte(rf12_status >> 8);
			printByte(rf12_status & 0xFF);
//			printByte((int) rxfill);
			Serial.print('!');
			printByte((int) n-RXFILL_MIN);
#endif
...
#endif
//        Serial.print(' ');
//        printByte((int) rf12_hdr);
        Serial.print(':');
        for (byte i = 0; i < n; ++i) {
            Serial.print(' ');
            printByte((int) rf12_buf[i]);
        }
        Serial.println();
#if 0
        // invert bits
        Serial.print("!INVERTED                    ");
        for (byte i = RXFILL_MIN; i < n; ++i) {
            Serial.print(' ');
            printByte((int) ((~(rf12_buf[i])) & 0xFF));
        }
        Serial.println();
#endif
#if 1
